...
def handle_connect_topic(message):
    ...
    chat = message.chat
    # Не даём привязать личку: иначе вебхук шлёт в user id → часто 403 "blocked by user"
    if getattr(chat, "type", None) == "private":
        bot.reply_to(
            message,
            "❌ <b>Нельзя привязать личные сообщения.</b>\n"
            "Откройте <b>супергруппу</b> с сигналами и отправьте "
            "<code>/connect_topic</code> в <b>нужной теме</b> (топике).",
            parse_mode="HTML",
        )
        return
    # Форум: без message_thread_id это «General», не отдельная тема
    if getattr(chat, "is_forum", False) and getattr(
        message, "message_thread_id", None
    ) is None:
        bot.reply_to(
            message,
            "❌ Это форум: зайдите в <b>нужную тему</b> и повторите "
            "<code>/connect_topic</code> там (не в общем чате).",
            parse_mode="HTML",
        )
        return
