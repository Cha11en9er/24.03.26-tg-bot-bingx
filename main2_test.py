...
class FundingMonitor:
    def __init__(self, symbols: list[str], interval_sec: float = 300.0):
        """
        symbols     — список тикеров в формате BingX: ["BTC-USDT", "ETH-USDT", ...]
...
if __name__ == "__main__":
    # 1) Сначала делаем единый запрос и вытаскиваем все символы
    all_data = fetch_all_funding()
    symbols = [item["symbol"] for item in all_data if item.get("symbol")]

    # 2) Передаём их в монитор — получим вывод по всем доступным тикерам
    monitor = FundingMonitor(symbols=symbols, interval_sec=300)

    # При желании можно блокировать ненужные:
    # monitor.block("APT-USDT")
